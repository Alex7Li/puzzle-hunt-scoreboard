...
for (let i = 0; i < sorted_teams.length; i++) {
  ...
  var trtbrow = tbl.insertRow();
  trtbrowr.insertCell().appendChild(document.createTextNode(i + 1));
  trtbrow.insertCell().appendChild(color_name(k));
  trtbrow.insertCell().appendChild(document.createTextNode(scores[k]));

  var find_time = last_egg_time[k];
  var team_thing_count = team_eggs[k];
  var gm_thing_count = scores[k] - team_thing_count;
  trtbrow.insertCell().appendChild(document.createTextNode(team_thing_count));
  trtbrow.insertCell().appendChild(document.createTextNode(gm_thing_count));
  trtbrow.insertCell().appendChild(document.createTextNode(find_time == null ? "\u2014" : find_time));
  trtbrow.insertCell().appendChild(document.createTextNode(team.hint_used ? "Used" : "Unused"));
}
